            ...
            {infoModal.isOpen && (
                <Modal title="알림" closeModal={() => setInfoModal({ isOpen: false, message: '' })}>
                    ...
                    <div className="text-right mt-6">
                        <button onClick={() => setInfoModal({ isOpen: false, message: '' })} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg font-medium">확인</button>
                    </div>
                </Modal>
            )}
            ...
            {confirmModal.isOpen && (
                <Modal title="삭제 확인" closeModal={() => setConfirmModal({ ...confirmModal, isOpen: false })}>
                    ...
                    <div className="flex justify-end gap-3 mt-6">
                        <button onClick={() => setConfirmModal({ ...confirmModal, isOpen: false })} className="bg-gray-300 hover:bg-gray-400 text-gray-800 px-5 py-2 rounded-lg font-medium">취소</button>
                        <button onClick={() => { confirmModal.onConfirm(); setConfirmModal({ isOpen: false, message: '', onConfirm: () => {} }); }} className="bg-red-600 hover:bg-red-700 text-white px-5 py-2 rounded-lg font-medium">삭제</button>
                    </div>
                </Modal>
            )}
        </div>
...
const SortControls = ({ sortOrder, onSortChange }) => (
    <div className="flex items-center gap-1">
        <button 
            ...
            className={`p-2 rounded-md transition-colors ${sortOrder === 'asc' ? 'bg-indigo-100 text-indigo-600' : 'text-gray-400 hover:bg-gray-200'}`}
            title="오름차순 정렬"
        >
            <ArrowUpIcon />
        </button>
        <button 
            ...
            className={`p-2 rounded-md transition-colors ${sortOrder === 'desc' ? 'bg-indigo-100 text-indigo-600' : 'text-gray-400 hover:bg-gray-200'}`}
            title="내림차순 정렬"
        >
            <ArrowDownIcon />
        </button>
            ...
            <div className="space-y-3">
                {classes.map(c => (
                    <div key={c.id} className="bg-white border rounded-lg p-3 flex items-center justify-between">
                        ...
                        <button onClick={() => onRequestDeleteClass(c.id)} className="p-2 text-red-600 hover:bg-red-100 rounded-full"><TrashIcon /></button>
                    </div>
                ))}
            </div>
        </Modal>
